...
export function startDashboard(getTelemetry: () => any, triggerFix: () => void) {
  const screen = blessed.screen({
    smartCSR: true,
    title: 'BCM4331 Forensic Controller',
    dockBorders: true
  });
  ...
  const telemetryTable = grid.set(9, 0, 3, 12, contrib.table, {
    ...
    columnSpacing: 2,
    columnWidth: [15, 10, 15, 10, 15, 10, 15, 10]
  });
  ...
  let timeLabels = Array(30).fill('').map((_, i) => `-${30 - i}s`);
